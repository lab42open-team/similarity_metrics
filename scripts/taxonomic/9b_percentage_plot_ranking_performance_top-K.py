...
def load_ranking_data(file):
    ...
    logging.info("Data from {} loaded correctly.".format(os.path.basename(file)))
    return df["Rank"]
...
def calculate_summary_statistics(files):
    ...
    euclidean_counts = calculate_counts(euclidean_ranks, total_ranks_e)
    cosine_counts = calculate_counts(cosine_ranks, total_ranks_c)
    jsd_counts = calculate_counts(jsd_ranks, total_ranks_j)
    
    return euclidean_counts, cosine_counts, jsd_counts
...
def plot_results(data, output_dir):
    ...
    plt.suptitle("Taxonomic", fontsize=14)
...
def save_results(output_file, noise_level, euclidean_counts, cosine_counts, jsd_counts):
    ...
    def write_counts(f, noise_level, metric, counts):
        f.write("{}\t{}\t{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}\n".format(
            noise_level, metric, counts["top_1"], counts["top_3"], counts["top_5"], counts["top_10"], counts["top_100"]
        ))
...
def main():
    ...
    downsampled_parent_dir = "/home1/nvenet/data/taxonomic/filtered_data/genus/noisy_versions/downsampled_noisy_version/ranking_output"
    output_dir = "/home1/nvenet/data/taxonomic/filtered_data/genus/noisy_versions/downsampled_noisy_version/plots_avg_performance"
