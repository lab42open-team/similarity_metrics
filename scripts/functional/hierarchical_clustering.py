...
def load_data(input_file, biome_file):
    try:
        ...
        logging.info("Distance matrix loaded successfully. Shape: {}".format(distance_matrix.shape))
...
"""Load function adjusted to drop data where biome is Unknown"""
def load_data(input_file, biome_file):
    try:
        df = pd.read_csv(input_file, sep="\t")
        biome_df = pd.read_csv(biome_file, sep="\t")
...
def perform_hierarchical_clustering(df, output_dir, input_file, k_values, stage="downsampled"):
    try:
        ...
        biome_color_map = {}
        for biome in df["biome_info"].unique():
            if "Aquatic" in biome:
                biome_color_map[biome] = "blue"
            elif "Terrestrial" in biome:
                biome_color_map[biome] = "limegreen"
            else:
                biome_color_map[biome] = "gray"  # fallback color
        ax = plt.gca()
        ...
        for k in k_values:
            ...
            if k == 2:
                # Filter only Cluster 1 samples
                cluster_1_samples = df[df["cluster"] == 1]
