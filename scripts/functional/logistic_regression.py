...
def train_model(x_train, y_train):
    try:
        ...
        model = LogisticRegression(multi_class="multinomial", solver="lbfgs", max_iter=500, n_jobs=4) # even without multi_class="multinomial", it chooses automatically by default the best strategy based on No of labels 
...
def cross_validate_model(x, y, label_mapping, output_file, cv=3):
    try:
        ...
        scores = cross_val_score(model, x, y, cv=skf, scoring='accuracy', n_jobs=4)
...
def main():
    data_dir = "/ccmri/similarity_metrics/data/functional/raw_data/GO_abundances/sim_metrics"
    biome_dir = "/ccmri/similarity_metrics/data/functional/raw_data/biome_info"
    data_file = os.path.join(data_dir, "c_distances_batches_normalized_f_aggregated_0.75_0.01_lf_v4.1_super_table.tsv")
    biome_file = os.path.join(biome_dir, "merged_biome_v4.1.tsv")
    model_path = "/ccmri/similarity_metrics/data/functional/raw_data/GO_abundances/logistic_regression/test_model_v4.1.pkl"
    scaler_path = "/ccmri/similarity_metrics/data/functional/raw_data/GO_abundances/logistic_regression/test_scaler_v4.1.pkl"
    evaluation_output_file = "/ccmri/similarity_metrics/data/functional/raw_data/GO_abundances/logistic_regression/evaluation_output_v4.1.txt"
    # Load data
