...
# Load the data from the two files - TEST files
file1 = "/ccmri/similarity_metrics/data/raw_data/lf_raw_super_table/filtered_data/genus/initial_data/v4.1_LSU_ge_filtered.tsv"
file2 = "/ccmri/similarity_metrics/data/raw_data/lf_raw_super_table/filtered_data/genus/noisy_versions_data/v4.1_LSU/normalized_counts/ra_0.2_v4.1_LSU_ge_filtered.tsv"
...
# Pivot the table to have taxa as rows and samples as columns
pivot_df = combined_df.pivot_table(index='Taxa', columns='Sample', values='Count', fill_value=0)

# Compute the Euclidean distance matrix
dist_matrix = squareform(pdist(pivot_df.T, metric='euclidean'))

# Convert distances to similarity scores (using inverse)
similarity_matrix = 1 / (1 + dist_matrix)

# Create a DataFrame for better readability
similarity_df = pd.DataFrame(similarity_matrix, index=pivot_df.columns, columns=pivot_df.columns)
